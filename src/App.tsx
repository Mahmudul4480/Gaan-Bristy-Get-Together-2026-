...
import { loadHonorableGuests } from './utils/guestStorage';
...
export default function App() {
  ...
  const [honorableGuests, setHonorableGuests] = useState<Ticket[]>(() => loadHonorableGuests());
  ...
  const refreshGuests = useCallback(() => {
    setHonorableGuests(loadHonorableGuests());
  }, []);
      ...
      <div className="bg-[#7A1F3D] text-[#F6EFE0] py-2 px-4 border-b border-[#D4AF37]/30 text-xs sm:text-sm font-semibold flex items-center justify-between gap-2 shadow-lg z-50 relative">
        <div className="max-w-7xl mx-auto w-full flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 overflow-hidden">
            <span className="bg-[#D4AF37] text-[#0F0C1A] text-[10px] font-black uppercase px-2 py-0.5 rounded-md animate-pulse shrink-0 shadow-sm">LIVE</span>
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button onClick={handleOpenGuestbook} className="bg-[#1C1730] hover:bg-[#D4AF37] hover:text-[#0F0C1A] text-[#F6EFE0] border border-[#D4AF37]/50 px-3 py-1 rounded-full text-xs font-bold transition flex items-center gap-1.5 cursor-pointer shadow-md group">
            ...
            <button onClick={() => setIsRegisterOpen(true)} className="btn-lighting px-3.5 py-1.5 rounded-full text-xs font-black bg-gradient-to-r from-[#F0D78C] via-[#D4AF37] to-[#F0D78C] cursor-pointer hidden md:flex items-center gap-1.5 shadow-[0_0_12px_rgba(212,175,55,0.4)] border border-[#F0D78C]">
              ...
              <span className="font-black font-bangla">টিকিট বুকিং</span>
            </button>
          </div>
        </div>
      </div>
