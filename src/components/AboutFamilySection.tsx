      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-8">
          {NOTABLE_MEMBERS.map((member) => (
            <article
              key={member.name}
              className="bg-[#1C1730] border border-[#D4AF37]/35 rounded-2xl p-5"
            >
              <div className="flex items-center gap-2 text-[#F0D78C] text-xs font-bold mb-2">
                {member.name === 'Artist Agun' ? (
              ...
              <h3 className="text-lg font-black text-[#F6EFE0]">{member.nameBengali}</h3>
              <p className="text-sm text-[#F0D78C] font-semibold">{member.name}</p>
              {member.starMakerId && (
